...
const DeployIt = () => {
  ...
  const downloadBlueprintPDF = () => {
    ...
    try {
      ...
      doc.text('AI AGENT BLUEPRINT', pageWidth / 2, 100, { align: 'center' });
      doc.setFontSize(16);
      doc.text(formData.agentName || 'Your AI Agent', pageWidth / 2, 120, { align: 'center' });
      doc.setFontSize(12);
      ...
      // Agent Configuration
      addText('AGENT CONFIGURATION', 16, true);
      addText(`Agent Name: ${formData.agentName}`, 12);
      addText(`Purpose: ${formData.purpose}`, 12);
      addText(`Target Audience: ${formData.whoItHelps}`, 12);
      if (formData.tone) addText(`Tone: ${formData.tone}`, 12);
      if (formData.tools.length > 0) addText(`Connected Tools: ${formData.tools.join(', ')}`, 12);
      if (formData.safetyRules) addText(`Safety Rules: ${formData.safetyRules}`, 12);
      yPosition += 10;

      // Implementation Blueprint
      addText('IMPLEMENTATION BLUEPRINT', 16, true);
      addText(blueprint.blueprint, 11);
      yPosition += 10;

      // Tips & Recommendations
      if (blueprint.tips) {
        addText('TIPS & RECOMMENDATIONS', 16, true);
        addText(blueprint.tips, 11);
        yPosition += 10;
      }

      // Resources
      if (blueprint.resources && blueprint.resources.length > 0) {
        addText('HELPFUL RESOURCES', 16, true);
        blueprint.resources.forEach((resource: string) => {
          addText(`• ${resource}`, 11);
        });
      }
      ...
      doc.save(`${formData.agentName.replace(/[^a-z0-9]/gi, '_')}_AI_Agent_Blueprint_${Date.now()}.pdf`);
