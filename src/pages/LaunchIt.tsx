...
const LaunchIt = () => {
  ...
  const downloadStrategy = () => {
    try {
      ...
      // Title
      addText('LAUNCH STRATEGY', 18, true);
      addText(`${formData.ideaCategory || 'Your Project'}`, 16, true);
      ...
      doc.save(`launch-strategy-${formData.ideaCategory?.toLowerCase().replace(/\s+/g, '-') || 'project'}-${Date.now()}.pdf`);
      toast.success('Launch strategy exported to PDF!');
    } catch (error) {
