...
${formData.businessName || "This business"} represents a compelling opportunity in the ${formData.industry || "market"} with strong potential for growth and profitability.
  ...
  const downloadPlan = () => {
    try {
      ...
      doc.text('BUSINESS PLAN', pageWidth / 2, 100, { align: 'center' });
      doc.setFontSize(18);
      ...
      addText('BUSINESS PLAN', 18, true);
      addText(`${formData.businessName || 'Your Business'}`, 16, true);
      addText(`Generated: ${new Date().toLocaleDateString()}`, 10);
      yPosition += 10;

      // Split plan into sections and add to PDF
      const sections = generatedPlan.split('\n\n');
      sections.forEach(section => {
        if (section.trim()) {
          const isHeader = section.toUpperCase() === section && section.length < 50;
          addText(section, isHeader ? 14 : 11, isHeader);
        }
      });
      ...
      doc.save(`business-plan-${formData.businessName?.toLowerCase().replace(/\s+/g, '-') || 'report'}-${Date.now()}.pdf`);
      toast.success('Business plan exported to PDF!');
    } catch (error) {
